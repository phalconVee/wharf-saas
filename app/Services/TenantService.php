...
class TenantService
{
    ...
    public function sendRegistrationNotifications(Tenant $tenant, string $domainWithHost): void
    {
        try {
            $tenant->notify(new NewSubscriptionNotification($domainWithHost));
        } catch (Throwable $exception) {
            Log::warning('Tenant welcome notification failed.', [
        ...
        try {
            ...
            Log::warning('Tenant registration admin notification failed.', [
                'tenant_id' => $tenant->getTenantKey(),
                'exception' => $exception->getMessage(),
            ]);
        }

      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header">
            <h3 class="card-title">{{ $t('loans.list.create.form_title') }}</h3>
          ...
          <form role="form" @submit.prevent="saveLoan" @keydown="form.onKeydown($event)">
            <div class="card-body">
              ...
              <div class="row">
                ...
                <div class="form-group col-md-4">
                  <label for="loanType">{{ $t('loans.common.loan_type')
                  }}<span class="required">*</span></label>
              ...
              <div class="row" v-if="form.authority && form.loanType == 0">
                ...
                <div class="form-group col-md-6">
                  <label for="availableAmount">{{
                    $t('loans.common.available_amount')
                  }}</label>
              ...
              <div v-if="form.loanType == 0" class="row">
                <div class="form-group col-md-6">
                  ...
                  <input id="amount" v-model="form.amount" type="number" step="any" class="form-control"
                    ...
                    :placeholder="$t('common.amount_placeholder')" min="1" :max="form.loanType == 0 ? form.authority.availableCCLoan : ''
              ...
              <div v-else class="row">
                <div class="form-group col-md-6">
                  ...
                  <input id="amount" v-model="form.amount" type="number" step="any" class="form-control"
                    :class="{ 'is-invalid': form.errors.has('amount') }" name="amount"
                    :placeholder="$t('common.amount_placeholder')" min="1" :max="form.loanType == 0 ? form.authority.availableAmount : ''
                      " @change="generatePayable" @keyup="generatePayable" />
                  <has-error :form="form" field="amount" />
                </div>
                <div class="form-group col-md-6">
...
export default {
  ...
  metaInfo() {
    return { title: this.$t('loans.list.create.page_title') }
  },
  data: () => ({
    breadcrumbsCurrent: 'loans.list.create.breadcrumbs_current',
    breadcrumbs: [
      {
        name: 'loans.list.create.breadcrumbs_first',
        url: 'home',
      },
      {
        name: 'loans.list.create.breadcrumbs_second',
        url: 'loans.index',
      },
      {
        name: 'loans.list.create.breadcrumbs_active',
        url: '',
      },
    ],
    form: new Form({
    ...
    async saveLoan() {
      await this.form
        ...
        .then(() => {
          toast.fire({
            type: 'success',
            title: this.$t('loans.list.create.success_msg'),
          })
